...
class Agent1(KartAgent):
    ...
    def choose_action(self, obs):
        if self.isEnd is False: # si le kart n'a pas fini la course
            ...
            print(f"Fini en {self.step_counter}") # affiche le nombre de pas pour finir la course

        if self.step_counter < 100: # on tourne tant qu'on a pas fait le demi tour
            steer = -1 # on tourne complétement à gauche
            acceleration = 0.11 # on laisse l'accelaration à 11%
            ...
            return action
        else: # on fait marche arrière jusqu'à la fin
            ...
            acceleration = 0.0 # on laisse l'accelaration à 0%
            brake = True # et on fait le marche arrière
